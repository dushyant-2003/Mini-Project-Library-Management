...
public class NotificationDAO extends GenericDAO<Notification>{
	...
	{	
		String sendNotificationQuery = String.format("insert into notification (notificationId, userId, type, message, date) values ('%s','%s','%s','%s','%s')", notification.getNotificationId(),notification.getUserId(),notification.getTitle(),notification.getMessage(),Date.valueOf(notification.getDate()));			
